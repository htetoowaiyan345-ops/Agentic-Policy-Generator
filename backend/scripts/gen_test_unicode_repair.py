#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Write tests/test_unicode_repair.py with proper UTF-8 encoding."""
from pathlib import Path

OUT = Path(r"D:\Htet Oo Wai Yan\OneDrive - City Holdings Limited\Desktop\agentic-policy-platform - Copy (4)\backend\tests\test_unicode_repair.py")
...
class TestTrimTrailingVirama(unittest.TestCase):
    def test_trailing_virama_before_space_removed(self) -> None:
        out = unicode_structural_repair("hello\u103A world")
        # The combination of trim-trailing-virama and reattach-combining
        # should produce no "\u103A" followed by a space at the end.
        last_virama_idx = out.rfind(VIRAMA)
        if last_virama_idx >= 0:
            tail = out[last_virama_idx:]
            self.assertFalse(tail.startswith(VIRAMA + " "))
