...
# Myanmar OCR fallback: if extraction looks corrupt, try OCR.
        try:
            ...
            if should_use_ocr(pdfplumber_doc.paragraphs, pdfplumber_doc.tables):
                ...
                # Layer A: hybrid PSM. Page 1 of Myanmar PDFs typically
                # carries the slot-1 header table; PSM=4 (single column of
                # text of variable sizes) preserves cell boundaries so the
                # 5-cell transposed label table (Policy no | Version |
                # Approved by | Prepared by | Effected/Review date)
                # remains readable as a header. Body pages 2+ use PSM=6
                # (uniform block) which is the default for prose. Both
                # modes run under LSTM (oem=1) and mya+eng language.
