...
def render(
    ...
    *,
    ...
    # Header label decoration: 12pt after-spacing on the two right-column
    # Header paragraphs that close their column groups — `Functional
    # Area(s)` and `Applies to` — followed by a 1px solid black top-border
    # on the NEXT visible paragraph so the line visually sits BELOW the
    # label rather than above it. Applied AFTER _normalize_word_format so
    # neither operation overwrites the other. Labels are looked up via
    # canonical_label() from BRAIN_HEADER_FIELDS so no string is hardcoded.
    _fa_label = canonical_label("Functional Area(s):")
    _at_label = canonical_label("Applies to:")
    if _fa_label and _at_label:
        _extra_after_labels = {_fa_label, _at_label}
        _body = doc.element.body
        # Collect body paragraphs (only top-level <w:p> children of body,
        # to preserve document order) and process them sequentially.
        _body_children = [
            ch for ch in list(_body) if ch.tag.endswith("}p")
        ]
        for _idx, _p in enumerate(_body_children):
            _p_text = "".join((t.text or "") for t in _p.iter(qn("w:t"))).strip()
            if not _p_text:
                continue
            _hit = None
            for _lbl in _extra_after_labels:
                if _p_text.startswith(_lbl):
                    _hit = _lbl
                    break
            if _hit is None:
                continue
            # 1) Boost w:after on the target paragraph (12pt breathing room).
            _pPr = _p.find(qn("w:pPr"))
            if _pPr is None:
                _pPr = OxmlElement("w:pPr")
                _p.insert(0, _pPr)
            _spacing = _pPr.find(qn("w:spacing"))
            if _spacing is None:
                _spacing = OxmlElement("w:spacing")
                _pPr.append(_spacing)
            _spacing.set(qn("w:after"), "240")
            # 2) Find the NEXT visible paragraph (skipping empties) and
            #    attach a top-border on it so the line visually appears
            #    BELOW the target label.
            _next_p = None
            for _j in range(_idx + 1, len(_body_children)):
                _cand = _body_children[_j]
                _cand_text = "".join((t.text or "") for t in _cand.iter(qn("w:t"))).strip()
                if _cand_text:
                    _next_p = _cand
                    break
            if _next_p is None:
                continue
            _npPr = _next_p.find(qn("w:pPr"))
            if _npPr is None:
                _npPr = OxmlElement("w:pPr")
                _next_p.insert(0, _npPr)
            if _npPr.find(qn("w:pBdr")) is None:
                _npBdr = OxmlElement("w:pBdr")
                _n_top = OxmlElement("w:top")
                _n_top.set(qn("w:val"), "single")
                _n_top.set(qn("w:sz"), "6")
                _n_top.set(qn("w:color"), "000000")
                # `w:space` on a paragraph top-border = distance from
                # the line to the paragraph's text content. Setting it
                # to 240 (12pt) gives 12pt of WHITE SPACE below the
                # line before the next paragraph's text starts. This
                # is symmetric with the 12pt `after=240` above the line
                # on the target label.
                _n_top.set(qn("w:space"), "240")
                _npBdr.append(_n_top)
                _npPr.append(_npBdr)
            # 3) Zero out w:before on the line-bearing next paragraph
            #    so the line + 12pt under-line gap sit flush against
            #    the start of this paragraph's content region.
            _n_spacing = _npPr.find(qn("w:spacing"))
            if _n_spacing is None:
                _n_spacing = OxmlElement("w:spacing")
                _npPr.append(_n_spacing)
            _n_spacing.set(qn("w:before"), "0")

    # Insert a 1px solid black horizontal line between consecutive named
    # slots 12 (DEFINITIONS) and 13 (RELATED POLICIES) in the .docx output.
    # A top border is added to the heading paragraph of slot 13 so the
    # boundary appears reliably even when slot 12 content length varies.
    _bounds = find_slot_boundaries(doc)
    if 12 in _bounds and 13 in _bounds:
        _s13_elems = _bounds[13].get("elements", [])
        if _s13_elems:
            _first = _s13_elems[0]
            if _first.tag.endswith("}p"):
                _pPr = _first.find(qn("w:pPr"))
                if _pPr is None:
                    _pPr = OxmlElement("w:pPr")
                    _first.insert(0, _pPr)
                if _pPr.find(qn("w:pBdr")) is None:
                    _pBdr = OxmlElement("w:pBdr")
                    _top = OxmlElement("w:top")
                    _top.set(qn("w:val"), "single")
                    _top.set(qn("w:sz"), "6")
                    _top.set(qn("w:space"), "1")
                    _top.set(qn("w:color"), "000000")
                    _pBdr.append(_top)
                    _pPr.append(_pBdr)
